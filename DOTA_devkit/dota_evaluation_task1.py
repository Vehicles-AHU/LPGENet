...
def voc_eval(detpath,
    ...
    fp = np.zeros(nd)
    for d in range(nd):
        img_index = str(image_ids[d]).zfill(5) # dronevehicle是，设置为5
        R = class_recs[img_index]
...
def main():
    ...
    # droneVehicle
    detpath = r'datasets/predictions_Txt/Task1_{:s}.txt' # 验证集predictions json转test
    annopath = r'datasets/val/{:s}.txt' # 验证集Labels
    imagesetfile = 'imgnamefile_val.txt'  # imgnamefile
    classnames = ['feright_car','car','truck','bus' ,'van']

    # VEDAI
    # detpath = r'datasets_vedai/predictions_Txt/Task1_{:s}.txt' # 验证集predictions json转test
    # annopath = r'datasets_vedai/labels/{:s}.txt' # 
    # imagesetfile = 'imgnamefile_vedai.txt'  # imgnamefile
    # classnames =  [
    #     "Car", "Truck", "Tractor", "CampingCar", 
    #     "Van", "Pickup", "Boat","Plane","Other"
    # ]
