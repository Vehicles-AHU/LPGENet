    ...
    try:
        ...
        if os.path.isfile(lb_file):
            ...
            with open(lb_file) as f:
                lb = [x.split() for x in f.read().strip().splitlines() if len(x)] # 每一行的数据，每一行9个数据，1（类别）+8（bbox）
                if any(len(x) > 6 for x in lb) and (not keypoint):  # is segment
                    classes = np.array([x[0] for x in lb], dtype=np.float32)
                    # labels中x1,y1,x2,y2,x3,y3,x4,y4 当成了分割的点。labels后八位转化为segments，即[4,2]的矩阵
                    segments = [np.array(x[1:], dtype=np.float32).reshape(-1, 2) for x in lb]  # (cls, xy1...)
                    # lb：labels后八位通过segments2boxes（实际是xyxy2xywh）方法，转化为xywh的格式
