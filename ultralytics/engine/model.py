...
# 定义了一个名为 Model 的类，继承自 nn.Module ，后者是PyTorch中用于构建神经网络模型的基类。
...
# 此类为与 YOLO 模型相关的各种操作提供通用接口，例如训练、验证、预测、导出和基准测试。它处理不同类型的模型，包括从本地文件、Ultralytics HUB 或 Triton Server 加载的模型。
    def __init__(
        self,
        model: Union[str, Path] = "yolo11n.pt", # 模型
        task: str = None,  # 任务：obb
        verbose: bool = False,
    ) -> None:
        super().__init__()
        ###########################################
        # 初始化 YOLO 模型类的新实例。
        ###########################################
        self.callbacks = callbacks.get_default_callbacks()  # 初始化 self.callbacks 属性，它使用 callbacks 模块的 get_default_callbacks() 函数来获取默认的回调函数。回调函数通常在训练过程中的特定事件（如每个epoch结束时）被调用，用于执行如日志记录、模型保存等操作。
        self.predictor = None  # 用于存储预测器对象，用于执行模型的预测。
        self.model = None  # 存储模型对象，即实际的神经网络模型
        self.trainer = None  # 存储训练器对象，用于管理模型的训练过程
        self.ckpt = {} # 存储检查点（checkpoint），即模型训练过程中的某个状态，用于恢复训练或进行模型评估。
        self.cfg = None # 存储配置文件，通常用于存储模型的配置参数。
        self.ckpt_path = None # 存储检查点文件的路径。
        self.overrides = {}   # 初始化训练参数覆盖字典。这个字典用于存储训练参数的覆盖值，可以在训练过程中动态修改训练器的行为。
        self.metrics = None   # 初始化度量属性。这个属性用于存储模型在验证和训练过程中的度量指标，如准确率、损失等。
        self.session = None   # 初始化HUB会话属性。这个属性用于存储与Ultralytics HUB的会话信息，用于与HUB进行交互，如上传模型、下载模型等。
        self.task = task   # 初始化任务类型属性。这个属性存储任务类型，可能用于确定模型的特定任务，如目标检测、图像分类等。
        self.model_name = None  # 模型名称
        # 处理模型路径字符串。将 model 参数转换为字符串，并去除可能的前后空白字符。
        model = str(model).strip()

        # 检查是否为 Ultralytics HUB模型。
...
# model.train方法
    def train(
        ...
        **kwargs: Any, # 额外的参数
        ...
        ################################################################
        # 1、检查模型是否为PyTorch模型。这行代码调用 _check_is_pytorch_model 方法来确保 self.model 是一个 PyTorch 模型。
        # 如果不是，将抛出 TypeError 异常。
        ###############################################################
        self._check_is_pytorch_model()

        ################################################################
        # 2、 处理Ultralytics HUB会话。如果存在一个 Ultralytics HUB 会话并且会话中有加载的模型，则使用 HUB 提供的训练参数，忽略本地提供的训练参数。
        #       hasattr(self.session, "model") : 默认是false
        #       self.session.model.id 检查 self.session 中的 model 对象是否有一个 id 属性，这通常意味着模型已经加载并且有一个唯一的标识符。
        #  一般情况下，这里的逻辑不回执行
        ################################################################
        ...
        # 4、加载配置文件和设置默认参数。
        ###############################################################      
        # 4.1 覆盖参数：cfg.yaml是否用了自定义配置参数文件，没有用的话返回默认参数（只有model和task）
        overrides = yaml_load(checks.check_yaml(kwargs["cfg"])) if kwargs.get("cfg") else self.overrides
        # 4.2 custom 字典定义了一些默认参数，包括数据集路径、模型路径和任务类型。
        custom = {
            "data": overrides.get("data") or DEFAULT_CFG_DICT["data"] or TASK2DATA[self.task],
            "model": self.overrides["model"],
            "task": self.task,
        } 
        # 4.3 合并参数。 args 字典合并了覆盖参数 overrides 、自定义参数 custom 、传入的关键字参数 kwargs ，并且添加了 "mode": "train" 参数，表示当前模式为训练模式。
        #   kwargs：是train方法额外传入的方法
        args = {**overrides, **custom, **kwargs, "mode": "train"}  # highest priority args on the right
        # 4.4 处理恢复训练。如果参数中包含 resume ，则设置恢复训练的检查点路径。
        if args.get("resume"):
        ...
        ################################################################
        # 5、初始化训练器，主要是调用task.py中的parse_model方法，构建模型结构
        ################################################################
        # 如果没有传入 trainer 参数或 trainer 参数为 None ，则调用 self._smart_load("trainer") 方法来智能加载一个默认的训练器。
        # 这里 self.trainer是 OBBTrainer对象
        self.trainer = (trainer or self._smart_load("trainer"))(overrides=args, _callbacks=self.callbacks)
        # 设置模型。如果不是恢复训练，则手动设置训练器的模型。
        if not args.get("resume"):  # 设置模型。如果不是恢复训练，则手动设置训练器的模型。
            self.trainer.model = self.trainer.get_model(weights=self.model if self.ckpt else None, cfg=self.model.yaml)
            self.model = self.trainer.model
        # 附加HUB会话。将可选的 HUB 会话附加到训练器。这里是None
        self.trainer.hub_session = self.session  # attach optional HUB session



        ################################################################
        # 6、模型实例加载完成，开始调用train
        # 1）OBBTrainer调用trainer，但是OBBTrainer没有train方法的实现，于是调用其父类DetectionTrainer的train
        # 2）DetectionTrainer的train也没有实现，再调用其父类BaseTrainer的train方法
        # 3）最后会调用val进行模型验证
        ################################################################
        self.trainer.train() #  self.trainer:OBBTrainer对象





        # 更新模型和配置。如果是单设备训练，更新模型和配置，加载最佳检查点或最后的检查点
        # 检查进程排名。 RANK 通常用于表示当前进程的编号。在单机训练中， RANK 通常是 -1 或 0 ，表示主进程或第一个进程。这段代码检查当前进程是否为主进程。
        if RANK in {-1, 0}:
