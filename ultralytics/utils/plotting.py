    ...
    def box_label(self, box, label="", color=(128, 128, 128), txt_color=(255, 255, 255), rotated=False):
        ...
        if self.pil or not is_ascii(label):
            ...
            if label:
                ...
                # 修改：将标签放在框的左边
                # 计算标签的位置 - 放在框的左侧
                x_left = max(0, p1[0] - w - 5)  # 向左偏移5像素，如果超出左边界则设为0
                y_top = p1[1]  # 与框的顶部对齐
                
                # 检查标签是否超出图像顶部
                outside = y_top >= h  # label fits outside box (top side)
                if not outside:  # 如果顶部空间不足
                    y_top = p1[1]  # 保持与框顶部对齐
                
                # 绘制标签背景矩形（在框的左边）
                self.draw.rectangle(
                    (x_left, y_top - h if outside else y_top, 
                     x_left + w + 1, y_top + 1 if outside else y_top + h + 1),
                    fill=color,
                )
                # 绘制标签文本
                self.draw.text((x_left, y_top - h if outside else y_top), 
            ...
            if label:
                ...
                # 检查标签是否超出图像顶部
                outside = p1[1] >= h  # label fits outside box (top side)
                if not outside:  # 如果顶部空间不足
                    y_top = p1[1]  # 保持与框顶部对齐
                else:
                    y_top = p1[1] - h  # 标签在框上方
                ...
                p2 = (x_left + w, y_top + h)
                cv2.rectangle(self.im, (x_left, y_top), p2, color, -1, cv2.LINE_AA)  # filled
                
                # 绘制标签文本
                text_y = y_top + h - 3 if outside else y_top + h - 2
                cv2.putText(
                    self.im,
                    label,
                    (x_left, text_y),
            ...
            if len(bboxes):
                ...
                for j, box in enumerate(boxes.astype(np.int64).tolist()):
                    ...
                    if labels or conf[j] > conf_thres:
                        ...
                        #     print(f"Error formatting confidence: {e}, conf: {conf}, j: {j}")
                        # label = ""
                        # label = f"{c} {conf[j]:.3f}"
                        # label = f"{''}" if labels else f"{''} {conf[j]:.2f}"
                        label = f"{c}"
